...
function ProjectDetails({ projects, expenses, incomes, categories, currentUser }) {
  const { id } = useParams();
  const navigate = useNavigate();
  const isAdmin = currentUser?.role === "admin" || currentUser?.userType === "admin";

  // Find the current Project by MongoDB _id (string match)
  ...
  // Filter expenses/incomes by role first (employee sees only their own)
  ...
  // Then filter by this specific project
  const projectExpenses = myExpenses
    .filter((e) => {
      const eProjId = e.projectID?._id || e.projectID;
      return String(eProjId) === String(id);
    })
    .map((e) => ({ ...e, type: "expense" }));

  const projectIncomes = myIncomes
    .filter((i) => {
      const iProjId = i.projectID?._id || i.projectID;
      return String(iProjId) === String(id);
    })
    .map((i) => ({ ...i, type: "income" }));
  ...
  const transactions = [...projectExpenses, ...projectIncomes].sort(
    (a, b) => {
      const dateA = new Date(a.expenseDate || a.incomeDate || 0);
      const dateB = new Date(b.expenseDate || b.incomeDate || 0);
      return dateB - dateA;
    }
  );
  ...
  // Helper for Category Name (populated: { _id, categoryName })
  const getCategoryName = (item) => item.categoryID?.categoryName || "-";
    ...
    <div className="container mt-5 fade-in-up">
      ...
      <div className="d-flex justify-content-between align-items-end mb-5 border-bottom pb-4" style={{ borderColor: 'var(--glass-border)' }}>
        <div>
          ...
          <p className="text-secondary mb-1" style={{ fontSize: '1.1rem' }}>{project.description || project.projectDetail || ""}</p>
          <small className="text-muted">
            {project.projectStartDate ? new Date(project.projectStartDate).toLocaleDateString() : "-"} — {project.projectEndDate ? new Date(project.projectEndDate).toLocaleDateString() : "Ongoing"}
          </small>
        </div>
            ...
            <tbody>
              {transactions.length === 0 ? (
                ...
                transactions.map((t, index) => (
                  <tr key={`${t.type}-${t._id || index}`}>
                    <td className="p-3 ps-4 text-secondary font-monospace small">
                      {t.expenseDate
                        ? new Date(t.expenseDate).toLocaleDateString()
                        : t.incomeDate
                    ...
                    <td className="p-3 text-white fw-medium">{getCategoryName(t)}</td>
                    <td className="p-3 text-secondary small">{t.description || t.expenseDetail || t.incomeDetail || "-"}</td>
